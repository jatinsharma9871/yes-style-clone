...
if(cart_items.length<1){
    ...
    cart_items.forEach(function(el,ind){
        ...
        var td2=document.createElement('select');
        for( var i=1;i<7;i++){
            var opt=document.createElement('option');
            opt.textContent=i;
            td2.add(opt);
        }
        td2.value=el.quantity;
        td2.addEventListener('change',function(){
            el.quantity=td2.value;
        ...
        textHolderDiv.append(price,title,category,quantity,td2,btnRemoveData);
    ...
    itemsTotalTxt.textContent = "Items Total ";
    var totalSumItems = "INR "+grossBill+".00";
    ...
    var estTotalFee = "INR "+grossBill+".00";
